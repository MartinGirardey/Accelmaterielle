...
def make_predictions(model, imagePath, threshold=config.PRED_THRESHOLD, binary=False, plot=False):
    ...
    with torch.no_grad():
        ...
        if binary:
            groundTruthPath = os.path.join(config.MASK_BINARY_DATASET_PATH, filename) # Binary
        else:
            groundTruthPath = os.path.join(config.MASK_MULTICLASS_DATASET_PATH, filename)  # Multiclass
        ...
        if binary:
            ...
            # print("{} ; {}".format(np.max(predMask), np.min(predMask)))
            ...
            predMask = np.zeros((predMasks.shape[1:]))
            predMask[:,:] = np.argmax(predMasks, axis=0)
            for i in range(config.NB_CLASSES):
                print("{} ; {}".format(np.max(predMasks[i,:,:]), np.min(predMasks[i,:,:])))
                predMask[predMask == i] = config.MASK_VALUES[i]
...
for path in imagePaths:
    ...
    # make_predictions(unet, path, threshold=0.5, binary=True, plot=True)
    make_predictions(unet, path, binary=False, plot=True)
